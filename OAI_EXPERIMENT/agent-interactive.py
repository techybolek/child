...
**Reminder**: First, retrieve and reason using relevant information from the vector store, determine if the retrieved information is relevant, then provide a clear, concise answer of 1-3 sentences. If no information is found, state this clearly.
...
async def main():
    print("Texas Childcare RAG Assistant")
    ...
    while True:
        ...
        with trace("cohort-2"):
            result = await Runner.run(
                ...
                context=TxChildcareRagContext(workflow_input_as_text=user_input)
            )

        # Extend history with agent's response items
