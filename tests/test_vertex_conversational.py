"""
Tests for Vertex Agent conversational support.

Validates multi-turn conversation functionality with thread-scoped memory.

Usage:
    pytest tests/test_vertex_conversational.py -v
    pytest tests/test_vertex_conversational.py -v -k "single_turn"  # Run specific test
"""
...
class TestVertexAgentConversational:
    """Tests for Vertex Agent multi-turn conversation support."""

    @pytest.fixture
    def handler(self):
        """Create handler instance for each test."""
        return VertexAgentHandler()
    ...
    def test_multi_turn_history_accumulation(self, handler):
        ...
        assert len(history) >= 4  # 2 user + 2 model (at minimum)
