"""
End-to-end test for the Bedrock Knowledge Base evaluation mode.
...
Usage:
    ...
    python tests/test_evaluation_e2e_bedrock.py  # Direct execution

Note: Bedrock KB mode has moderate latency (~3-8s per question) so timeout is extended.
"""
...
MODE = "bedrock"
RUN_NAME = "TEST_BEDROCK"
MIN_EXPECTED_QUESTIONS = 3
MIN_PASS_SCORE = 0.0  # Bedrock KB may not have test docs loaded; validate pipeline works
...
def bedrock_configured():
    ...
    Checks for valid AWS credentials that can access Bedrock services.
    """
    try:
        import boto3
...
def run_all_tests():
    ...
    if not bedrock_configured():
        print("SKIPPED: AWS credentials not configured for Bedrock")
        return True
